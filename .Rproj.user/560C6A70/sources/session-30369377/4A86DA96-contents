...
preview_multimedia <- dbGetQuery(con, sprintf("SELECT CoreId, rightsHolder, creator, accessURI FROM parquet_scan('%s')", sas_url_mutlimedia))
...
filtered <- dbGetQuery(duckdb_con, sprintf("
  SELECT country, count(1) FROM parquet_scan('%s') WHERE scientificName = 'Tubaria furfuracea' GROUP BY country ", URL_OCCURRENCE_PARQUET))
